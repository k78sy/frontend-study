...
//     //console.log(event);
//     //영역 내 x,y 좌표: offsetX, offsetY
//     ctx.beginPath(); // 이걸 안해주면 모두 한path로 잡혀서 동시에 색바뀜.

//     ctx.moveTo(0, 0); // moveTo로 처음을 잡아줘야 다음부터 바로 ㄱ
//     const color = colors[Math.floor(Math.random() * colors.length)] //colors의 랜덤 컬러 가져오기. floor : 올림하기
...
//canvas.addEventListener("click", onClick); 첫번째 단계. 클릭으로 해보기
//canvas.addEventListener("mousemove", mouseMove); // 두번째 단계, 마우스 움직일때부터 그리기
